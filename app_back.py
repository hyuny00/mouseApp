...
def select_words():
    ...
    random_entries = random.sample(list(second_column), 12)
...
def on_start_button():
    ...
    for record in records:
        if (
            record["event"] != "enter"
            # and record["event"] != "Main Event"
            and record["event"] != "sleep"
        ):
...
def on_mouse_button_release(event):
    global is_dragging, selected_color, t
    description = description_entry.get()
    x, y = event.x_root, event.y_root  # Capture coordinates
    if description or selected_action in ["click", "color", "Main Event"]:
...
action_combobox = ttk.Combobox(
    button_frame, values=["click", "input", "color", "Main Event"], width=10
)
